...
export default function Raises() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const raisesData = data.raises || [];
        const sorted = Array.isArray(raisesData) 
          ? [...raisesData].sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0))
          : [];
        setRaises(sorted);
      } catch (err) {
        console.error("Raises API error:", err);
  ...
  const filteredRaises = useMemo(() => {
    if (!search) return raises;
    const searchLower = search.toLowerCase();
    return raises.filter((r) =>
      r.name?.toLowerCase().includes(searchLower) ||
      r.round?.toLowerCase().includes(searchLower)
    );
  }, [raises, search]);

  const totalPages = Math.ceil(filteredRaises.length / rowsPerPage);
  const paginatedRaises = filteredRaises.slice(
  ...
  if (error) {
    return (
      <div className="bg-[#22222f] border border-[#2d2d3d] rounded-xl p-8 text-center">
        <div className="text-white font-semibold mb-2">Fundraising Data Requires Pro API</div>
        <div className="text-[#6b7280] text-sm mb-4">
          DeFiLlama has moved fundraising analytics to their Pro tier.
        </div>
        <a
  ...
  return (
    <div className="space-y-6">
      <div className="relative max-w-md">
        ...
        <input
          type="text"
          placeholder="Search projects, rounds..."
          value={search}
      ...
      <div className="bg-[#22222f] border border-[#2d2d3d] rounded-xl p-6 shadow-lg overflow-hidden">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xs font-semibold text-[#6b7280] uppercase tracking-wide">Crypto Fundraising</h2>
          <ExportCsvButton data={paginatedRaises} filename="raises.csv" />
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left text-[11px] text-[#6b7280] uppercase tracking-wide border-b border-[#2d2d3d]">
                ...
                <th className="py-3 px-4 font-semibold">Amount Raised</th>
                <th className="py-3 px-4 font-semibold">Round</th>
                <th className="py-3 px-4 font-semibold">Date</th>
                <th className="py-3 px-4 font-semibold">Lead Investors</th>
              </tr>
            </thead>
            <tbody>
              {paginatedRaises.map((raise, index) => {
                const globalIndex = (currentPage - 1) * rowsPerPage + index + 1;
                const investors = raise.leadInvestors || raise.investors || [];
                return (
                  <tr key={`${raise.name}-${raise.date}-${index}`} className="border-b border-[#2d2d3d] last:border-b-0 hover:bg-[#2d2d3d]/30 transition-colors">
                    <td className="py-4 px-4 text-[#6b7280]">{globalIndex}</td>
                    <td className="py-4 px-4 text-white font-medium">{raise.name || "N/A"}</td>
                    <td className="py-4 px-4 text-right text-white font-semibold">
                      ${((raise.amount || 0) / 1e6).toFixed(1)}M
                    </td>
                    <td className="py-4 px-4 text-[#6b7280]">
                      <span className="inline-block px-2.5 py-1 bg-[#2d2d3d] text-[#a97bd1] text-xs rounded-md">
                        {raise.round || "N/A"}
                      </span>
                    </td>
                    <td className="py-4 px-4 text-[#6b7280]">
                      {raise.date ? new Date(raise.date).toLocaleDateString() : "N/A"}
                    </td>
                    <td className="py-4 px-4 text-[#6b7280] text-xs">
                      {investors.slice(0, 2).join(", ")}{investors.length > 2 ? ` +${investors.length - 2}` : ""}
                    </td>
                  </tr>
                );
              })}
