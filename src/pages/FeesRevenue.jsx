...
import { fetchFees, formatNumber } from "@/utils/defiApi";
import ExportCsvButton from "@/components/ExportCsvButton";

export default function FeesRevenue() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#22222f] border border-[#2d2d3d] rounded-xl p-6 shadow-lg">
        <div className="text-xs font-semibold text-[#6b7280] uppercase tracking-wide mb-2">Total Fees 24h</div>
      ...
      <div className="bg-[#22222f] border border-[#2d2d3d] rounded-xl p-6 shadow-lg overflow-hidden">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xs font-semibold text-[#6b7280] uppercase tracking-wide">Protocols by Fees</h2>
          <ExportCsvButton data={sortedProtocols} filename="fees-revenue.csv" />
        </div>
        <table className="w-full">
          <thead>
            <tr className="text-left text-[11px] text-[#6b7280] uppercase tracking-wide border-b border-[#2d2d3d]">
              ...
              <th className="py-3 px-4 font-semibold text-right cursor-pointer hover:text-[#a97bd1] transition-colors" onClick={() => handleSort("total24h")}>Fees 24h</th>
              <th className="py-3 px-4 font-semibold text-right cursor-pointer hover:text-[#a97bd1] transition-colors" onClick={() => handleSort("total7d")}>Fees 7d</th>
              <th className="py-3 px-4 font-semibold text-right cursor-pointer hover:text-[#a97bd1] transition-colors" onClick={() => handleSort("total30d")}>Fees 30d</th>
            </tr>
          </thead>
          <tbody>
