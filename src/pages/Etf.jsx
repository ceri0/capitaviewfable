...
export default function Etf() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const res = await fetch("https://api.llama.fi/overview/etfs");
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const result = await res.json();
        setData(result);
        const etfData = result.protocols || result.etfs || [];
        setEtfs(Array.isArray(etfData) ? etfData : []);
      } catch (err) {
        console.error("ETF API error:", err);
  ...
  const sortedEtfs = useMemo(() => {
    return [...filteredEtfs].sort((a, b) => (b.inflowUSD ?? 0) - (a.inflowUSD ?? 0));
  }, [filteredEtfs]);

  const totalPages = Math.ceil(sortedEtfs.length / rowsPerPage);
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-3 gap-6">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="h-20 bg-[#22222f] border border-[#2d2d3d] rounded-xl animate-pulse"></div>
          ))}
        </div>
  ...
  if (error) {
    return (
      <div className="bg-[#22222f] border border-[#2d2d3d] rounded-xl p-8 text-center">
        <div className="text-white font-semibold mb-2">ETF Data Requires Pro API</div>
        <div className="text-[#6b7280] text-sm mb-4">
          DeFiLlama has moved ETF analytics to their Pro tier.
        </div>
        <a
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-6">
        <div className="bg-[#22222f] border border-[#2d2d3d] rounded-xl p-5 shadow-lg">
          <div className="text-[11px] text-[#6b7280] uppercase tracking-wide mb-2">Total ETF Volume 24h</div>
      ...
      <div className="bg-[#22222f] border border-[#2d2d3d] rounded-xl p-6 shadow-lg overflow-hidden">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xs font-semibold text-[#6b7280] uppercase tracking-wide">ETF Inflows & AUM</h2>
          <ExportCsvButton data={paginatedEtfs} filename="etf.csv" />
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left text-[11px] text-[#6b7280] uppercase tracking-wide border-b border-[#2d2d3d]">
                ...
                <th className="py-3 px-4 font-semibold">ETF Name</th>
                <th className="py-3 px-4 font-semibold text-right">24h Inflow</th>
                <th className="py-3 px-4 font-semibold text-right">7d Inflow</th>
                <th className="py-3 px-4 font-semibold text-right">Total AUM</th>
              </tr>
            </thead>
            <tbody>
              {paginatedEtfs.map((etf, index) => {
                const globalIndex = (currentPage - 1) * rowsPerPage + index + 1;
                return (
                  <tr key={etf.name || index} className="border-b border-[#2d2d3d] last:border-b-0 hover:bg-[#2d2d3d]/30 transition-colors">
                    <td className="py-4 px-4 text-[#6b7280]">{globalIndex}</td>
                    <td className="py-4 px-4 text-white font-medium">{etf.name || "N/A"}</td>
                    <td className={`py-4 px-4 text-right font-semibold ${(etf.total24h ?? 0) >= 0 ? "text-[#34d399]" : "text-red-400"}`}>
                      ${((etf.total24h || 0) / 1e6).toFixed(2)}M
                    </td>
                    <td className={`py-4 px-4 text-right font-semibold ${(etf.total7d ?? 0) >= 0 ? "text-[#34d399]" : "text-red-400"}`}>
                      ${((etf.total7d || 0) / 1e6).toFixed(2)}M
                    </td>
                    <td className="py-4 px-4 text-right text-white font-semibold">
                      ${((etf.totalAum || 0) / 1e9).toFixed(2)}B
                    </td>
                  </tr>
                );
              })}
